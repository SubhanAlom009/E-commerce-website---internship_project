    ...
    <div className='flex flex-col mx-8 mt-24 mb-12'>
        <div className='flex justify-center gap-24'>
            ...
            <div className='flex flex-col justify-center w-1/2 space-y-4'>
                ...
                <p className='text-2xl'>Quantity: <input className='w-12 p-2 border-2 rounded-lg ' type="number" min={1} max={10} defaultValue={1}/></p>
                <button className='bg-[#0a1a3d] text-[#F5F5F5] p-3 rounded-lg hover:bg-[#2560e1] transition-all duration-150'>Add to Cart</button>
                <button className='bg-[#0a1a3d] text-[#F5F5F5] p-3 rounded-lg hover:bg-[#2560e1] transition-all duration-150'>Buy Now</button>
            </div>
        </div>
            ...
            <div className='grid grid-cols-3 gap-4 '>
                {
                    filterSimilarProduct.map((product,index)=>{
                        return (
                            <Link to={`/products/${product.id}/${product.name}`} className='flex flex-col justify-between p-5 bg-[#F5F5F5] border border-slate-300 cursor-pointer shadow-[0_3px_10px_rgb(0,0,0,0.2)] hover:scale-95 transition-all duration-150'  key={index}>
